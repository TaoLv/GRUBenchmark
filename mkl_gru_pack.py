...
class GRU_Inference(gof.Op):
    ...
    def c_code(self, node, name, inputs, outputs, sub):
        ...
        ccode += """
            ...
            A[2] = hinit_ptr;
tic = dsecnd();
            B[0] = cblas_%(dtype)sgemm_alloc(CblasBMatrix, m[0], n[0], k[0]);     // w_hz
            B[1] = cblas_%(dtype)sgemm_alloc(CblasBMatrix, m[0], n[0], k[0]);     // w_hr
            B[2] = cblas_%(dtype)sgemm_alloc(CblasBMatrix, m[0], n[0], k[0]);     // w_hh

            cblas_%(dtype)sgemm_pack(CblasRowMajor, CblasBMatrix, CblasNoTrans, m[0], n[0], k[0], alpha[0], wh_z, n[0], B[0]);
            cblas_%(dtype)sgemm_pack(CblasRowMajor, CblasBMatrix, CblasNoTrans, m[0], n[0], k[0], alpha[0], wh_r, n[0], B[1]);
            cblas_%(dtype)sgemm_pack(CblasRowMajor, CblasBMatrix, CblasNoTrans, m[0], n[0], k[0], alpha[0], wh_h, n[0], B[2]);
...
printf(\"pack: %%.8f\\n\", (dsecnd() - tic) * 1000.0);
            ...
            for (int i = 0; i < time_step; i++) {
                ...
                tic = dsecnd();
                cblas_%(dtype)sgemm_compute(CblasRowMajor, CblasNoTrans, CblasPacked, m[0], n[0], k[0], A[0], k[0], B[0], n[0], beta[0], C[0], n[0]);
                cblas_%(dtype)sgemm_compute(CblasRowMajor, CblasNoTrans, CblasPacked, m[0], n[0], k[0], A[1], k[0], B[1], n[0], beta[0], C[1], n[0]);
                acc1 += dsecnd() - tic;
                ...
                tic = dsecnd();
                cblas_%(dtype)sgemm_compute(CblasRowMajor, CblasNoTrans, CblasPacked, m[0], n[0], k[0], A[2], k[0], B[2], n[0], beta[0], C[2], n[0]);
                acc3 += dsecnd() - tic;
            ...
            tic = dsecnd();
            cblas_%(dtype)sgemm_free(B[0]);
            cblas_%(dtype)sgemm_free(B[1]);
            cblas_%(dtype)sgemm_free(B[2]);
            printf(\"free: %%.8f\\n\", (dsecnd() - tic) * 1000.0);
