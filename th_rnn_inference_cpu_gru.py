...
from mkl_gru_seq import GRU_Inference
...
def build_model():
    x = T.ftensor3('inp')
    wx = theano.shared(np.random.rand(3 * input_size, hidden_size).astype(np.float32))
    wh = theano.shared(np.random.rand(3 * hidden_size, hidden_size).astype(np.float32))
    hs = theano.shared(np.zeros((batch_size, hidden_size), np.float32))

    out = GRU_Inference(hid=hidden_size, return_sequences=True, max_len=10)(x, wx, wh, hs)
    model = theano.function(inputs=[x], outputs=out[0][-1])
